...
class App extends Component {
  ...
  decrementCartItemQuantity = id => {
    ...
    const findThecardItem = cartList.find(
      eachCardItem => eachCardItem.id === id,
    )
    if (findThecardItem.quantity > 1) {
  ...
  addCartItem = product => {
    ...
    if (findThecardItem === undefined) {
      this.setState(prevState => ({cartList: [...prevState.cartList, product]}))
    } else {
      this.setState(prevState => ({
        cartList: prevState.cartList.map(eachItem => {
          if (eachItem.id === findThecardItem.id) {
